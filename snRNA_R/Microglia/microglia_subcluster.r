...
#seu<-readRDS("/media/tsun/Data/Tsun/Ting/Myelin_mutant/RDS/2020Oct_MyelinMutants_all_cleaned_run2.rds")

#subset microglia
mic<-subset(seu, subset = CellType %in% c("Microglia"))
...
p2<-match("Plp1",rownames(exp))
p3<-match("Mbp",rownames(exp))

exp_noko<-exp[-c(p1,p2,p3),]
...
mic_new<-RunUMAP(mic_new, dims = 1:20)
...
#test runs suggested clustering at resolution 0.3

mic <- FindClusters(object = mic, resolution = 0.3)
table(Idents(mic))
...
mic@meta.data$CellType<-factor(mic@meta.data$CellType,
                              ...
                              labels = c("Homeostatic", "Homeostatic",
                                        "Myelin processing", "Brain Border \n Marcrophages",
                                        "DAM","unknown"))
...
#as major discussion point in the manuscript
#the following script focus on analyzing CnpKO, MbpKO and coresponding WT

mic<-subset(seu, subset = Genotype %in% c("CnpKO", "Foxg1MbpKO","WT"))

mic

#after subsetting, re normalize, sclae and embed data
DefaultAssay(mic)<-"RNA"
...
#embedding for 3 selected genotypes, with kockout genes
DimPlot(mic, group.by = "Genotype")
DimPlot(mic, group.by = "CellType", cols = c("tomato","olivedrab3",
                                           "turquoise3","steelblue3","peru"))
...
sep_pro<-subset(sep_pro,subset = Genotype %in% c("WT","CnpKO","Foxg1MbpKO"))
...
color_oder <- setNames(
    ...
    c("Homeostatic", "Myelin processing", "Brain Border \n Marcrophages",
                                        "DAM","unknown"))

DimPlot(mic, group.by = "CellType",
        cols = c("tan1","lightskyblue","lightcoral","lightseagreen","grey48"),
...
sampleID_list<-vector(mode = "list", length = 3)
